...
description: Security overview of the OpenSigner trust model covering key share distribution, authentication providers, transport encryption, and TLS requirements.
---
...
- If the attacker only has the user's credentials but no device access, the outcome depends on how cold share recovery is handled:
    ...
    - With *automatic recovery*:
        - Without OTP: the attacker can recover the user's secret key by using the stolen credentials.
...
- **[Custody and key reconstruction](/security/deployment-scenarios#custody-and-key-reconstruction)** - Backend reconstruction and non-custodial guarantees
...
Transport security relies on [TLS 1.3](https://datatracker.ietf.org/doc/html/rfc8446).

<JsonLd data={techArticle({ headline: 'Security overview', description: 'Security overview of the OpenSigner trust model covering key share distribution, authentication providers, transport encryption, and TLS requirements.', path: '/security/overview' })} />
