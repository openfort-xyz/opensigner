...
description: Threat analysis of OpenSigner covering iframe compromise, auth service attacks, token forgery, and storage tampering with detailed mitigations and best practices.
---
...
If the following components are tampered with by a third party or compromised while under the developer's control,
...
because the cold storage share is encrypted with user entropy, and access to the cold storage
...
This analysis follows established practices such as [OWASP threat modeling](https://owasp.org/www-community/Threat_Modeling) and [STRIDE](https://learn.microsoft.com/en-us/azure/security/develop/threat-modeling-tool-threats).

## Related

- Read the [security overview](/security/overview) for the trust model.
- Choose a configuration in [deployment scenarios](/security/deployment-scenarios).

<JsonLd data={techArticle({ headline: 'Threat analysis', description: 'Threat analysis of OpenSigner covering iframe compromise, auth service attacks, token forgery, and storage tampering with detailed mitigations and best practices.', path: '/security/threat-analysis' })} />
