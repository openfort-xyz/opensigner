...
description: Complete API reference for the OpenSigner Cold Storage (Shield) with Swagger docs covering error codes for OTP, projects, shares, users, and authentication.
---
...
- See [cold storage (Shield)](/components/shield) and [OTP for automatic recovery](/components/cold_storage/otp).

This reference is generated from an [OpenAPI](https://www.openapis.org/) definition.

See [recovery methods](/security/recovery-methods) for how cold shares are recovered. API docs render with [Swagger UI](https://swagger.io/tools/swagger-ui/).

<JsonLd data={techArticle({ headline: 'Cold storage API', description: 'Complete API reference for the OpenSigner Cold Storage (Shield) with Swagger docs covering error codes for OTP, projects, shares, users, and authentication.', path: '/apis/cold_storage' })} />
