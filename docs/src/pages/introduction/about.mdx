...
1. **[iFrame](/components/iframe)**: Performs user operations, handles key splitting, and reconstructs keys.
2. **Key Share Storage**: Keys are split into three shares, stored in different locations:
    ...
    - **[Hot storage](/components/hot_storage)**: External storage for frequently accessed shares.
    - **[Cold storage](/components/shield)**: External storage for shares accessed only when a local or hot share is missing.
3. **[Authentication Service](/components/auth)**: Verifies user access to shares by issuing access tokens to the iFrame and exposing validation endpoints to the storages.
...
Ready to try it? Continue with [Setup](/introduction/setup), then [Getting started](/introduction/getting-started). For deeper background on threat models and deployment choices, see [Security overview](/security/overview).
