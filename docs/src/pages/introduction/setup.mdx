...
The project Makefile builds and runs all components through docker-compose. For a primer on what OpenSigner is and the problem it solves, start with [What is OpenSigner?](/introduction/about).

Building the images takes time, particularly the Better Auth component used for [authentication service](/components/auth) database migrations: [`@better-auth/cli`](https://www.npmjs.com/package/@better-auth/cli).
...
Once you have everything running, head over to the [Getting Started](/introduction/getting-started) guide. If you plan to self-host only some components, review the trade-offs in [Deployment scenarios](/security/deployment-scenarios).
