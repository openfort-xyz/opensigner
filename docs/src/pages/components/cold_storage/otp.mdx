...
OTP verification follows the standard one-time password schemes, [HOTP (RFC 4226)](https://datatracker.ietf.org/doc/html/rfc4226) and [TOTP (RFC 6238)](https://datatracker.ietf.org/doc/html/rfc6238).

## Related

- OTP is part of [automatic recovery](/security/recovery-methods).
- It runs against [cold storage (Shield)](/components/shield) when [creating a key](/actions/signup).

<JsonLd data={techArticle({ headline: 'OTP for automatic recovery', description: 'One-Time Password (OTP) verification for automatic recovery adds an additional layer of security when creating encrypted sessions.', path: '/components/cold_storage/otp' })} />
