...
This is the HTTP API reference for the [cold storage (Shield) component](/components/shield). It pairs with the [hot storage API](/apis/hot_storage) and uses the same [authentication service](/components/auth). OTP-protected endpoints are explained in [OTP for automatic recovery](/components/cold_storage/otp).
