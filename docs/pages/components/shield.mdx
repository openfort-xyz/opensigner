...
description: Explore the OpenSigner Shield cold storage component built in Go with MySQL, supporting password, passkey, and automatic recovery with OTP for key shares.
---
...
Shield relies on the same authentication system as the [Hot Storage](/components/hot_storage). That is, an OIDC-compatible [authentication system](/components/auth) needs to be set up and specified for Shield to be able to recognize and validate users.
...
Shield is fully written in Go and it relies on a MySQL-compatible database for its persistence layer.
...
Shield requires a MySQL-compatible database. **Shield uses MariaDB for development.** Configure the following environment variables
...
This type of share recovery can be enhanced with OTP (One-Time Password) verification. See [OTP for Automatic Recovery](/components/cold_storage/otp) for detailed documentation, and the [Recovery methods](/security/recovery-methods) page for a comparison with password and passkey recovery. The full HTTP surface lives in the [Cold storage API](/apis/cold_storage) reference.
