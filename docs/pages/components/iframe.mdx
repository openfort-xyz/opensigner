...
description: Deep dive into the OpenSigner iFrame, the browser-based component that handles private key generation, Shamir share splitting, and ephemeral reconstruction.
---

# iFrame

The iFrame is the core client-side component that handles all cryptographic operations securely within the user's browser.
...
and storing them in their respective storage components ([hot storage](/components/hot_storage) and [cold storage](/components/shield)); as well as fetching the shares
and reconstructing the private key when required. See [Recovery methods](/security/recovery-methods) for how the cold share is encrypted before storage.
...
The constructor expects the access token and the URL of the hot storage.
...
For end-to-end flows that exercise the iFrame, see [Create a key](/actions/signup), [Recover a key](/actions/login), and [Use a key](/actions/operation). The threat model around iframe compromise is covered in [Threat analysis](/security/threat-analysis).
