...
[Shield](/components/shield) supports One-Time Password (OTP) verification to add an additional layer of security when creating encrypted sessions.

:::info
OTP is available only for shares with the [automatic recovery method](/security/recovery-methods#automatic-recovery).
:::
...
These workflows correspond to the [Create a key](/actions/signup#otp-with-automatic-recovery) and [Recover a key](/actions/login#otp-with-automatic-recovery) flows. The full endpoint reference lives in the [Cold storage API](/apis/cold_storage).
