...
The [iframe](/components/iframe) is the only component with access to the full, reconstructed key.
...
Having a compromised [auth service](/components/auth) has, besides the usual implications,
...
If the [hot storage](/components/hot_storage) is compromised or tampered with, attackers have access to one
...
The risk of a compromised [cold storage](/components/shield) is, in isolation, lesser than that of the hot storage
...
**Mitigations:**
- Use [password or passkey recovery](/security/recovery-methods) (user-held secrets)
- [Enable OTP](/components/cold_storage/otp) for automatic recovery
