...
The `entropy` field indicates how the share was encrypted on Openfort (for example, `passkey`, `password`, or `project`). You need to use the same recovery method when recovering the key in OpenSigner.
...
3. Select the **Recovery Method** that matches the `entropy` value from the cold share export:
   - **Project Entropy** for shares encrypted with project-level encryption
   - **Password** for shares encrypted with a user password
   - **Passkey** for shares encrypted with a passkey (WebAuthn PRF)
