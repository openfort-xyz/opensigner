...
Depending on whether the user has automatic key recovery or
password based key recovery:

## User with Password Recovery

The user tries to recover the key trough the iFrame. For that, it'll attempt to reconstruct its key
...
Thats why on this diagram showed only encrypted session creation flow.
