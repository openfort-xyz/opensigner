...
The user tries to recover the key through the iFrame. For that, it attempts to reconstruct its key
and fails due to not having the local share. This share is stored in each device after the user recovers it in that device for the first time.
...
Usually, `Admin` is the developer of the application the user is using, and `User` is the end user.
:::
...
private key reconstruction happening in the cold storage ambiguous. The following section explains it in more detail.
