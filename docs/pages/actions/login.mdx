...
description: Learn how OpenSigner recovers private keys on new devices using password, passkey, or automatic recovery with OTP through the iFrame and cold storage workflow.
---
...
When the admin calls the cold storage `/v2/devices/register` endpoint, it provides
