...
To create a new key with OpenSigner, users call the `create()` method on the [iFrame](/components/iframe).
The iFrame generates a new private key and splits it into three shares using threshold cryptography.
Two shares are distributed to [hot storage](/components/hot_storage) and [cold storage](/components/shield) respectively, while the third share is stored locally in the localStorage.
...
You can [enable OTP verification](/components/cold_storage/otp) for your Shield project to enhance the security of automatic recovery shares.
...
After creation, the same key can be retrieved via [Recover a key](/actions/login) and used to sign messages via [Use a key](/actions/operation). For a full comparison of the three recovery options, see [Recovery methods](/security/recovery-methods).